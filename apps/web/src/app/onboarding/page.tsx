    ...
    <div className="flex items-center gap-0">
      {steps.map((s, i) => (
        <div key={s.n} className="flex items-center">
          ...
          {i < steps.length - 1 && (
            <div
              className="w-16 h-px mb-5 mx-2"
              style={{
                backgroundColor: current > s.n ? "#4ade8040" : "#2a2a2a",
              }}
            />
          )}
        </div>
...
function BlueskyConnect({ onConnected }: { onConnected: () => void }) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "#111111", border: "1px solid #5b63d340" }}
    >
...
export default function OnboardingPage() {
  ...
  const stepParam = parseInt(
    searchParams.get("step") ?? (billingEnabled ? "1" : "2"),
  );
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        const { planStatus, trialEndsAt } = await apiFetch<{
          planStatus: string;
  ...
  async function checkout(planId: string) {
    ...
    try {
      const { url } = await apiFetch<{ url: string }>("/billing/checkout", {
        ...
        body: JSON.stringify({
          planId,
          successUrl: "/onboarding?step=2&trialStarted=1",
        }),
      });
  ...
  const planPrice = (p: (typeof PLANS)[number]) =>
    ...
    >
        ...
        <div className="w-full max-w-3xl">
          <div className="text-center mb-8">
            ...
            <p className="text-sm" style={{ color: "#555" }}>
              Choose a plan and enter your card — you won&apos;t be charged
          ...
          <p className="text-center text-xs" style={{ color: "#333" }}>
            14 days free · card required · cancel any time ·{" "}
            {isIndia ? "billed in INR" : "billed in USD"}
          </p>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="w-full max-w-lg">
          ...
          {oauthError && (
            <div
              className="px-4 py-3 rounded-2xl mb-6 text-sm"
              style={{
                backgroundColor: "#1a0a0a",
                border: "1px solid #3a1a1a",
                color: "#f87171",
          ...
          <div className="space-y-3 mb-6">
            ...
            <a
              href={`${API_BASE}/auth/threads?from=onboarding`}
            ...
            <a
              href={`${API_BASE}/auth/instagram?from=onboarding`}
                      ...
                      return (
                        <button
                          ...
                          style={
                            selected
                              ? {
                                  backgroundColor: "#5b63d320",
                                  color: "#ededed",
                                  border: "1px solid #5b63d360",
                                }
                              : {
                                  backgroundColor: "#1a1a1a",
                                  color: "#555",
              ...
              >
                ...
                <div className="flex justify-end">
                  <span
                    className="text-xs"
                    style={{
                      color: postText.length > 280 ? "#fb923c" : "#444",
                    }}
                  >
                    {postText.length} / 300
                  </span>
          ...
          <div className="flex flex-col gap-3">
            {accounts.length > 0 && (
              <button
                ...
                disabled={
                  scheduling || !postText.trim() || selectedIds.length === 0
                }
                className="w-full py-3 rounded-xl text-sm font-semibold transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-100 active:scale-[0.98]"
                style={{ backgroundColor: "#ffffff", color: "#0a0a0a" }}
              >
