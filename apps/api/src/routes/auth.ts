...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/auth/instagram/callback", async (req, reply) => {
    ...
    // Fetch profile from graph.instagram.com
    // Note: for accounts affected by the Meta bug (short-lived fallback), this also fails with code 100.
    // In that case displayName stays as the numeric igUserId — acceptable until Meta fixes their API.
